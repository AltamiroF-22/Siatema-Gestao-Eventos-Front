<template>
      ...
      <v-row v-else>
        <v-col v-for="event in myEvents" :key="event.id" cols="12" md="6" lg="4">
          <v-card>
            ...
            <v-card-actions>
              <v-btn color="primary" :disabled="loading" @click="editEvent(event)">
                <v-icon left>mdi-pencil</v-icon> Editar
              </v-btn>
  
              <v-btn color="red" :disabled="loading" @click="deleteEvent(event.id)">
  ...
  const deleteEvent = async (eventId: number | undefined) => {
    if (!eventId) return;
    const confirmDelete = confirm("Tem certeza que deseja apagar este evento?");
    if (confirmDelete) {
      console.log("Apagando evento:", eventId);
      // Chamar API para deletar o evento
    }
  };
